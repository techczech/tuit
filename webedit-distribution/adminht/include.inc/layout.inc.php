  ...
  <td class="topright" rowspan=2>
    <table width="100%">
      ...
      <tr>
        ...
        {
          $array = explode('<!--INSERTEWCONTROLHERE-->',$MODULES[$m]['mainright']);
//          $EWCONTROL->setImagePath("$CONFIGURATION[server_addr]/","bohemica.com/admin/images/");
          $EWCONTROL->setImagePath("$CONFIGURATION[server_addr]/","images/");
//	  $EWCONTROL->setPath("/www/doc/www.bohemica.com/www/adminew");
	  for($i=0;$i<count($array)-1;$i++)
	  {
	    echo $array[$i];
	    $EWCONTROL->ShowControl($EWNAMES[$i]);
	  }
	  echo $array[$i];
	  $EWCONTROL->ShowFinish();
        }
        else
